...
def load_data():
    ...
    with open("data/domains.csv", newline='') as csvfile:
    ...
    headers = []
    with open("data/inspect.csv", newline='') as csvfile:
        for row in csv.reader(csvfile):
    ...
    headers = []
    with open("data/tls.csv", newline='') as csvfile:
        for row in csv.reader(csvfile):
            if (row[0].lower() == "domain"):
                headers = row
                continue
    ...
    headers = []
    with open("data/analytics.csv", newline='') as csvfile:
        for row in csv.reader(csvfile):
            if (row[0].lower() == "domain"):
                headers = row
                continue
...
def https_row_for(domain):
    ...
    if (https <= 0):
        ...
        # 1 year and up is a yes.
            ...
            # This kind of "Partial" means `includeSubdomains`, but no
            # `preload`.
...
def save_tables():
    ...
    analytics_agencies_path = os.path.join(
        TABLE_DATA, "analytics/agencies.json")
